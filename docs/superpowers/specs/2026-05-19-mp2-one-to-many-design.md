...
└── support/
    ...
    └── (수정) EntityMetadata.java           (oneToManies: List<CollectionMetadata> 필드 추가)
    └── (수정) EntityMetadataAnalyzer.java   (@SfsOneToMany 분기 + generic 추출, fail-fast 3종)
    └── (수정) EntityPersister.java          (buildRowMapper에 oneToManies 채우기 + findAll 메서드)
...
### 3.5 `EntityPersister` 확장 + `findAll` API

```java
// EntityPersister.buildRowMapper — oneToManies 채우기 추가 (LAZY only)
...
public List<Object> findAll(PersistenceContext context) {
    String sql = "SELECT " + columnsForSelect() + " FROM " + md.tableName();
    return jdbc.query(sql, buildRowMapper(context));
    // 각 row → buildRowMapper → context.putEntity (cache hit이면 재사용, 정점 ② 정합)
}
```

> **`buildRowMapper`의 cache hit 재사용 보강** — Phase 4의 EAGER 분기에 이미 있는 패턴(`getEntity → 없으면 loadById → putEntity`)을 *findAll의 각 row 매핑*에도 *명시 박제*. 정점 ② 1 entity = 1 instance 보장. (구현 디테일은 plan task에서 박제)
...
public class DefaultCollectionLoader implements CollectionLoader {
    ...
    public <T> List<T> loadCollection(Class<T> elementType, String fkColumn, 
        ...
        String sql = "SELECT " + persister.columnsForSelect() 
                   + " FROM " + persister.metadata().tableName()
                   + " WHERE " + fkColumn + " = ?";
        @SuppressWarnings("unchecked")
        List<T> result = (List<T>) jdbc.query(sql, persister.buildRowMapper(ctx), fkValue);
        return result;
    }
}
```
...
public class SqlCountingJdbcTemplate extends JdbcTemplate {
    ...
    public SqlCountingJdbcTemplate(DataSource ds) { super(ds); }
...
| 통합 테스트 | 시나리오 (회귀 박제) |
|---|---|
| `OneToManyLazyIntegrationTest` | (1) 첫 List 메서드 호출 시 정확히 1 SELECT 발생 — `size()` 호출 검증 / (2) PC close 후 List 메서드 호출 시 `SfsLazyInitializationException` |
| `NPlusOneIntegrationTest` | (1) `findAll(User)` (N=3) + for-loop `u.getOrders().size()` → spy 카운트 정확히 N+1 = 4 / (2) for-loop 후 *4번째 user* find 시 *기존 identityMap hit*으로 SELECT 추가 안 발생 |
...
3. **`EntityMetadata.oneToManies`** 필드 추가 (`List<CollectionMetadata>`)
4. **`EntityMetadataAnalyzer`** `@SfsOneToMany` 분기 + generic 자동 추출 + fail-fast 3종
5. **`SfsPersistentList<T> implements List<T>`** — 모든 메서드 lazy init trigger + closed 예외 + isInitialized 헬퍼
6. **`CollectionLoader` 인터페이스** + **`DefaultCollectionLoader`** 구현
7. **`EntityPersister.buildRowMapper`** oneToManies 채우기 (SfsPersistentList stub 주입)
8. **`EntityPersister.findAll`** + **`RealEntityManager.findAll`** + **`SfsEntityManager.findAll`** 시그니처
...
|---|---|---|
| `EntityMetadata(class, table, idField, idGen, columns, manyToOnes, ... )` | **+ `oneToManies` 필드** 추가 | 모든 생성자 호출 지점 정정 필요 (`EntityMetadataAnalyzer.doAnalyze` 1곳) |
| `EntityMetadataAnalyzer.doAnalyze` | 분기 추가, 기존 분기 그대로 | 회귀 영향 0 (분기 추가만) |
...
| `JdbcTemplate` (sfs-tx) | 변경 *없음* | spy는 테스트 전용 서브클래스 |
