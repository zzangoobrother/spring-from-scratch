...
- [x] **Step 1: `AppConfig.java`에 `@ComponentScan` 추가**
...
- [x] **Step 2: `TodoDemoApplication.java` 축약**
...
- [x] **Step 3: 본 plan DoD 18항목 모두 `[x]` + 실행 기록 블록 추가**
...
- [x] 1. `ConfigurableBeanFactory`에 `registerDisposableBean` + `containsSingleton` 시그니처 추가 (Task A1)
- [x] 2. `ConfigurableListableBeanFactory`에 `resolveDependency` 시그니처 추가 (Task A1)
- [x] 3. `DefaultListableBeanFactory.resolveDependency`에 `@Override` 적용 (Task A1)
- [x] 4. `AutowiredAnnotationBeanPostProcessor` 생성자 파라미터 → `ConfigurableListableBeanFactory` (Task A1)
- [x] 5. `CommonAnnotationBeanPostProcessor` 생성자 파라미터 → `ConfigurableBeanFactory` (Task A1)
- [x] 6. `AnnotationConfigUtils` 다운캐스팅 블록 제거 (Task A1)

**byte-buddy 인프라:**

- [x] 7. `BeanMethodInterceptor` 신설 + 단위 테스트 3건 (Task B1)
- [x] 8. `ConfigurationClassEnhancer` 신설 + 단위 테스트 2건 (Task B2)
...
- [x] 16. `TodoDemoApplication` 7 클래스 명시 등록 → `AppConfig.class` + `@ComponentScan` 축약 (Task D3)

**품질:**

- [x] 17. `./gradlew build` 전체 PASS + 누적 145 PASS / 0 FAIL (Task E1)
- [x] 18. `sfs-samples/README.md` Phase 2A 갱신 사항 + 마일스톤 표 추가 (Task E1)

---

> **실행 기록 (2026-04-26):**
>
> - **회귀:** sfs-core 28 + sfs-beans 58 + sfs-context 55 + sfs-samples 8 = **총 145 PASS / 0 FAIL** (Phase 1C 종료 138 → +7)
> - **카운트 정정:** plan Step 1 메모는 +5 추정(B1 3, B2 2, C2 2)이었으나 실측 +7. 차이의 정체는 *C1 풀 사이클 reviewer 후속*에서 추가된 단위 테스트 2건(`ConfigurationClassEnhancer` null 가드 / final 가드) + 통합 테스트 1건(`Phase1IntegrationTest.directCallFormBypassesEnhanceWhenProxyBeanMethodsDisabled`) — spec 작성 시점에 예측하지 못한 reviewer 발견.
> - **빌드:** `./gradlew build` → BUILD SUCCESSFUL
> - **커밋 수:** task 작업 16 커밋 + spec/plan docs 2 커밋 = 총 18 커밋 (plan 추정 9를 초과 — C1이 풀 사이클 reviewer 반영으로 5 커밋, B1/B2가 코드 리뷰 후속으로 각 3/2 커밋이 된 결과)
> - **편차 박제 1건:** `ConfigurationClassEnhancer`의 ClassLoadingStrategy를 `WRAPPER` → `UsingLookup(privateLookupIn(...))`로 교체. Java 25 unnamed module + inner static class 환경에서 `IllegalAccessError` 해소. Spring 본가 CGLIB 동일 메커니즘이라 학습 가치 보존.
> - **편차 박제 1건:** `ComponentScanFromConfigurationTest.componentScanValueAliasIsHonored`의 `ValueAliasConfig`를 메서드 내 로컬 클래스 → `static class`로 변경. 로컬 non-static 클래스의 *implicit outer reference* 생성자 파라미터 때문에 byte-buddy 서브클래스 인스턴스화 실패. 학습 함의: enhance 대상은 *진짜 무인자 default constructor*가 필요.
> - **마일스톤:** `EnhanceAbsenceDemo` → `ConfigurationEnhanceDemo`로 rename되며 `account.user == ctx.user` 직접 호출 형태가 `→ false`(Phase 1C 박제) → `→ true`(Phase 2A 회수)로 변형. 코드 한 줄 안 바꾸고 *진실이 뒤집힘*을 박제(테스트)가 자동 검출.
> - **Phase 2A 종료** — 마감 게이트 진입 준비 완료

> **품질 게이트 기록 (2026-04-26):**
>
> **1단계 — 다관점 코드리뷰** (Phase 2A 변경 17 커밋 전체 대상)
> - Reviewer A (동작 중심: 아키텍처/테스트/라이프사이클): ⚠️ 1건, ✅ 2건. 즉시 고칠 P1 1건 + P2 1건, 이월 3건.
> - Reviewer B (코드 중심: 가독성/주석/보안): A− (8.5/10). 즉시 고칠 P2 1건 + P3 2건, 이월 3건.
> - 통합: 즉시 고칠 5건, 이월 5건 (그 중 1건은 이미 박제 완료).
>
> **2단계 — 리팩토링** (3 커밋, 동작 무변경)
> - `refactor(sfs-beans):` AbstractAutowireCapableBeanFactory 다운캐스팅 `DefaultListableBeanFactory` → `ConfigurableListableBeanFactory` 정리 + containsSingleton Javadoc 박제 (커밋 `841d3f2`)
> - `refactor(sfs-context):` ConfigurationClassPostProcessor 주석 정리 — processComponentScans sanitize 박제 추가 + enhanceConfigurationClasses 인라인 주석 한 단락 통합 (커밋 `8a4983f`)
> - `refactor(sfs-context):` Phase1IntegrationTest try-with-resources 적용 — 4개 테스트 메서드 일관성 확보. `phase1FullScenario`는 close 후 어서션이 있어 Worker 참조를 try 밖에 선언하고 어서션은 블록 후에 두는 패턴 사용 (편차 없음) (커밋 `2545b8b`)
>
> **3단계 — `/simplify` 패스** (3 reviewer agent 병렬: 재사용 / 코드 품질 / 효율)
> - Agent 1 (재사용): 3 finding (P2 #1, P2 #2, P3 #3). #1·#3 거부 (학습 가치 손실), #2는 *교차 참조 주석으로 절충*.
> - Agent 2 (코드 품질): 1 finding (P3) — `mergePackages` 주석 WHAT 절반 단축. 나머지 7개 카테고리 모두 정상 (마감 게이트 1+2단계 효과).
> - Agent 3 (효율): 0 finding — 학습 프로젝트 hot path는 부트스트랩 1회뿐, 우연한 redundancy 없음.
> - 통합 처리: 1 커밋 — `refactor(sfs-context):` 주석 정리 (mergePackages WHAT 단축 + resolveBeanName 교차 참조 박제) (커밋 `b53d316`)
> - *전수 수용 금지* 원칙 적용: 4 finding 중 2건만 반영 (50%). DRY/일관성 같은 일반 원칙을 *학습 의도*가 능가하는 케이스 다수.
>
> **회귀:** 145 PASS / 0 FAIL 유지. `./gradlew build` BUILD SUCCESSFUL.
> **이월 4건 (Phase 2B 진입 시 처리):** @Bean(name="...") 통합 박제, paramType.getSimpleName() 의존성 이름 전략, byte-buddy args 미사용 박제, named module 전환 박제.
>
> **품질 게이트 통과 — main 머지 진입 준비 완료.** 누적 변경: task 16 + 마감 게이트 5 + spec/plan docs 2 = 총 23 커밋.
