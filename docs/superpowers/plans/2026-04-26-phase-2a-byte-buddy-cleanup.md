...
- [ ] **Step 3: `BeanMethodInterceptor.java` 구현**
...
- [ ] **Step 1: 통합 테스트 어서션 + 메서드명 갱신 (실패 상태로 만들기)**
...
- [ ] **Step 2: 테스트 실행 (FAIL — enhance 미구현, 여전히 isNotSameAs 결과가 나옴)**
...
- [ ] **Step 3: `ConfigurationClassPostProcessor`에 `enhanceConfigurationClasses` 추가**
...
- [ ] **Step 2: 테스트 실행 (FAIL — `@ComponentScan` 클래스 없음)**
...
- [ ] **Step 4: `ConfigurationClassPostProcessor`에 `processComponentScans` 추가**
...
- [ ] **Step 1: `EnhanceAbsenceDemo.java` rename + 본문 출력문구 갱신**
...
- [ ] **Step 2: `EnhanceAbsenceDemoTest.java` rename + 어서션 갱신**
...
- [ ] 9. `enhanceConfigurationClasses()` 추가 — `proxyBeanMethods=true`인 `@Configuration` BD의 beanClass를 enhance 클래스로 교체 (Task C1)
- [ ] 10. `Phase1IntegrationTest.directCallFormCreatesDistinctInstanceWithoutEnhance` 메서드명 + 어서션 + Javadoc 갱신 (Task C1)
- [ ] 11. `@ComponentScan` 애노테이션 신설 (`value`/`basePackages` 동의어) (Task C2)
- [ ] 12. `processComponentScans()` 추가 — `@Configuration` 클래스의 `@ComponentScan` 발견 시 `ClassPathBeanDefinitionScanner.scan` 호출 (Task C2)
- [ ] 13. `ComponentScanFromConfigurationTest` 통합 테스트 2건 신설 (Task C2)

**`sfs-samples` 갱신:**

- [ ] 14. `UserService` IdGenerator/Clock 통일 — `@Autowired Clock` 제거, `idGen.nowInstant()` 사용 (Task D1)
- [ ] 15. `EnhanceAbsenceDemo` → `ConfigurationEnhanceDemo` rename + 출력문구 갱신 + 테스트 어서션 갱신 (Task D2)
