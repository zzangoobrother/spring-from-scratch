...
- [x] **Step 3: `flush`에 orphanRemoval 단계 추가**
...
> **실행 기록 (2026-05-26):** `SfsPersistentList.findOrphans()`가 plan에 package-private으로 정의되어 있었으나 `RealEntityManager`(다른 패키지)에서 접근 시 컴파일 에러 발생. `public`으로 변경해 해결. 기존 `SfsPersistentListTest`는 같은 패키지이므로 영향 없음. 커밋에 `SfsPersistentList.java`를 추가로 포함.
