...
class RealEntityManagerPersistTest {
    ...
    // ds/tsm을 클래스 필드로 추출 — 테스트 메서드에서 emf2 빌드 시 재사용
    ...
    void setup() {
        ...
        // IDENTITY persist_id_test 테이블 초기화 (G2 — 테스트 격리)
    ...
    void persist_with_IDENTITY_strategy_immediately_inserts_and_assigns_id() {
        // IDENTITY 전략 엔티티 전용 emf2 빌드
        SfsEntityManagerFactory emf2 = SfsEntityManagerFactory.builder()
                .dataSource(ds)
                .transactionSynchronizationManager(tsm)
                .addEntityClass(IdOrder.class)
                .build();
        RealEntityManager em = (RealEntityManager) emf2.createEntityManager();
